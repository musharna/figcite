    ...
    for pno in range(doc.page_count):
        ...
        for info in page.get_images(full=True):
            ...
            try:
                ...
                rects = []
            yield {"page": pno + 1, "xref": xref, "blob": blob,
                   "rects": [fitz.Rect(r) for r in rects]}


def match_image(blob: bytes, manifest: dict[str, Record],
                fuzzy_distance: int = 6) -> tuple[Optional[Record], str]:
    ...
    if hit:
        ...
        fuzzy.note = (fuzzy.note + f" [matched perceptually, hamming={dist}; PDF export "
                                   "re-encodes images so sha256 cannot match]").strip()
...
def audit(pdf_path: str | Path, min_inches: float = 1.0) -> dict:
    ...
    return {
        ...
        "untagged_substantive": len([r for r in rows
                                     if r["record"] is None and not r["decorative"]]),
        "rows": rows,
    }
...
def apply(pdf_path: str | Path, out_path: str | Path, *,
    ...
    try:
        ...
        if missing:
            ...
            note = (f"{len(missing)} image(s) on page(s) "
    ...
    return {"out": str(out_path), "pictures": len(rows), "cited": len(entries),
            "unsourced": len(missing), "manifest": written, "entries": entries,
            "rows": rows}
...
def _fit_textbox(page, rect, text: str, fontsize: float, fontname: str = "helv",
                 color=None, min_fontsize: float = 5.0) -> float:
...
def _caption(page, rect, text: str) -> None:
    ...
    try:
        ...
        _fit_textbox(page, box, text[:60] + "...", CAPTION_PT, "helv", GRAY,
                     min_fontsize=4.0)
...
def _credits_pages(doc, entries: list[str], missing_note: str) -> int:
    ...
    def new_page(n):
        ...
        used = _fit_textbox(page, fitz.Rect(m, m, m + width, m + 40),
                            title, CREDITS_TITLE_PT, "hebo")
        y = m + used + 10
    ...
    for line in entries:
        ...
        if y + needed > bottom:
            new_page(pages + 1)
        used = _fit_textbox(page, fitz.Rect(m, y, m + width, y + needed + 4),
                            line, CREDITS_BODY_PT, "helv")
        y += used + 6
    if missing_note:
        note = "! " + missing_note
        ...
        if y + needed > bottom:
            new_page(pages + 1)
        _fit_textbox(page, fitz.Rect(m, y, m + width, y + needed + 4),
                     note, CREDITS_BODY_PT, "helv", RED)
    return pages
...
def _write_manifest(path: str | Path, rows: list[dict]) -> dict:
    ...
    cols = ["n", "page", "matched_by", "confirmed", "doi", "short_cite", "citation",
            "license_url", "reuse", "source_kind", "captured_local", "context"]
    with open(stem + ".csv", "w", newline="", encoding="utf-8") as fh:
        ...
        for r in rows:
            rec = r["record"]
            w.writerow({
                "n": r["n"], "page": r["page"], "matched_by": r["matched_by"],
