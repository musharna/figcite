...
from figcite.browser import (doi_from_publisher_pattern, doi_from_url_text,
                             doi_from_page_meta, firefox_profiles,
                             lookup_by_title, resolve_from_capture,
                             snapshot_history, strip_browser_suffix, url_to_doi)
...
REAL_URL_SHAPES = [
    ...
    ("https://link.springer.com/content/pdf/10.1038/msb.2013.40.pdf", "10.1038/msb.2013.40"),
...
def test_publisher_pattern_and_its_negative_control():
    ...
    assert doi_from_publisher_pattern(
        "https://www.nature.com/articles/s41598-019-42976-3") == "10.1038/s41598-019-42976-3"
...
def test_strip_browser_suffix():
    assert strip_browser_suffix(
        "Plant room in 3D - plant twin - Mozilla Firefox") == "Plant room in 3D - plant twin"
...
def test_real_firefox_history_is_readable_and_discriminates():
    ...
    row = con.execute("select title from moz_places where title is not null "
                      "and url like 'http%' order by last_visit_date desc limit 1").fetchone()
    con.close()
    if not row:
...
def test_unknown_title_is_not_grounded_and_says_why():
    capture = {"title": "figcite definitely not a real page zzqq - Mozilla Firefox",
               "process": "firefox"}
    res = resolve_from_capture(capture, allow_fetch=False)
...
def test_real_identifier_paths_resolve_and_discriminate():
    ...
    assert doi_from_alternative_id(
    ...
    assert doi_from_ncbi_id(
        "https://pmc.ncbi.nlm.nih.gov/articles/PMC7140940/") == "10.1104/pp.19.01474"
...
def test_nearest_visit_match_is_never_grounded(monkeypatch):
    ...
    monkeypatch.setattr(B, "lookup_by_time", lambda db, ts, window_s=180: {
        "url": real_url, "title": "whatever", "visited": "", 
        "match": "nearest-visit", "ambiguous": True})

    cap = {"title": "Some page - Mozilla Firefox", "process": "firefox",
    ...
    monkeypatch.setattr(B, "lookup_by_title", lambda db, t: {
        "url": real_url, "title": t, "visited": "", 
        "match": "exact-title", "ambiguous": False})
