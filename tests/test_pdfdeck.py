...
from figcite.provenance import Record, dhash_bytes, embed, hamming, read_embedded, sha256_bytes
...
def test_textbox_that_does_not_fit_is_never_silently_dropped():
    ...
    assert used > 0
    assert "Image credits" in page.get_text(), "the helper reported success but nothing was drawn"
...
def test_credits_paginate_without_losing_entries(tmp_path):
    ...
    entries = [
        f"[{i}] Author{i} et al. (20{10 + i % 15}). "
        + "A rather long title " * 4
