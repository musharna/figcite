...
from figcite.provenance import (Record, dhash_bytes, embed, hamming,
                                read_embedded, read_sidecar, sha256_bytes)
...
def test_png_roundtrip_carries_doi(tmp_path):
    ...
    rec = Record(doi="10.1038/s41586-020-2649-2", citation="Harris et al. (2020). Nature 585:357-362.",
                 short_cite="Harris et al. 2020", confirmed=True)
...
def test_xmp_packet_present(tmp_path):
    src = _png(tmp_path, "src.png")
    rec = Record(doi="10.1111/mec.12953", citation="Ho et al. (2014).", title="A title",
                 license_url="https://creativecommons.org/licenses/by/4.0", confirmed=True)
    out = tmp_path / "x.png"
    embed(src, out, rec)
...
def test_jpeg_carries_cite_but_is_honest_about_the_limit(tmp_path):
    ...
    rec = Record(doi="10.1038/x", citation="Someone et al. (2001). A journal 1:2-3.", confirmed=True)
    out = tmp_path / "o.jpg"
    embed(src, out, rec)
    back = read_embedded(out.read_bytes())
